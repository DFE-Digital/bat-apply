var APPLY = window.APPLY || {};
    ...
    APPLY.StepByStep = function () {
      ...
      var activeLinkClass = 'apply-step-nav__list-item--active';
      var activeStepClass = 'apply-step-nav__step--active';
      ...
      (function ($element) {
        ...
        $element.addClass('apply-step-nav--active');
  
        // Prevent FOUC, remove class hiding content
        $element.removeClass('js-hidden');
  
        stepNavSize = $element.hasClass('apply-step-nav--large') ? 'Big' : 'Small';
        ...
        var totalLinks = $element.find('.apply-step-nav__link').length;
        ...
        function addShowHideAllButton() {
          $element.prepend('<div class="apply-step-nav__controls"><button aria-expanded="false" class="apply-step-nav__button apply-step-nav__button--controls js-step-controls-button">' + actions.showAllText + '</button></div>');
        }
          ...
          $stepHeaders.each(function() {
            ...
            if (!$(this).find('.js-toggle-link').length) {
              $(this).find('.js-step-title-button').append(
                '<span class="apply-step-nav__toggle-link js-toggle-link" aria-hidden="true" hidden></span>'
              );
            }
          });
        }
        ...
        function addButtonstoSteps() {
          $.each($steps, function () {
            ...
            $title.wrapInner(
              '<button ' +
              'class="apply-step-nav__button apply-step-nav__button--title js-step-title-button" ' +
        ...
        function setActiveStepClass() {
          ...
          $element.find('.' + activeLinkClass).closest('.apply-step-nav__step').addClass(activeStepClass).attr('data-show', "");
        }
      ...
      function componentLinkClick(event, stepNavTracker, linkPosition) {
        ...
        function trackClick() {
          ...
          var dimension28 = $(event.target).closest('.apply-step-nav__list').attr('data-length');
